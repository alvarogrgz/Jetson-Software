...
class IRGBDCameraSimulator {
 public:
    explicit IRGBDCameraSimulator(std::shared_ptr<crf::sensors::rgbdcameras::RGBDCameraMock> mock) :
        ...
        iso_(1200) {
            ...
            ON_CALL(*mock_, getRgbdFrame()).WillByDefault(testing::Invoke([this]() {
                if (!initialized_) return cv::rgbd::RgbdFrame();
                if (failsToGetFrame) return cv::rgbd::RgbdFrame();
                cv::rgbd::RgbdFrame frame;
                frame.image = cv::Mat(resolution_, CV_8UC3);
                cv::randu(frame.image, cv::Scalar(0, 0, 0), cv::Scalar(255, 255, 255));

                frame.depth = cv::Mat(depthResolution_, CV_16U);
            ...
            ON_CALL(*mock_, getAvailableFramerates(testing::_)).WillByDefault(testing::Invoke([this](const cv::Size& resolution) {
                return std::vector<int>({6, 15, 20, 25, 30});
            }));
            ...
            ON_CALL(*mock_, setDepthResolution(testing::_)).WillByDefault(testing::Invoke([this](const cv::Size& res) {  // NOLINT
                if (failsSetResolution) return false;
                if (!initialized_) return false;

                depthResolution_ = res;
                return true;
            }));

            ON_CALL(*mock_, setDepthFramerate(testing::_)).WillByDefault(testing::Invoke([this](int fps) {  // NOLINT
                if (failsSetFramerate) return false;
                if (!initialized_) return false;
            ...
            ON_CALL(*mock_, getDepthResolution()).WillByDefault(testing::Return(depthResolution_));
            ON_CALL(*mock_, getDepthFramerate()).WillByDefault(testing::Return(depthFramerate_));

            ON_CALL(*mock_, getAvailableDepthResolutions()).WillByDefault(testing::Invoke([this]() {
                return std::vector<cv::Size>({
                    cv::Size(100, 100),
                    cv::Size(150, 150),
                    cv::Size(640, 480),
            ...
            ON_CALL(*mock_, getAvailableDepthFramerates(testing::_)).WillByDefault(testing::Invoke([this](const cv::Size& resolution) {
                return std::vector<int>({6, 15, 20, 25, 30});
            }));

            ON_CALL(*mock_, getColorCameraMatrix()).WillByDefault(
                testing::Return(cv::Mat(3, 3, CV_32F)));
            ON_CALL(*mock_, getColorDistortionMatrix()).WillByDefault(
                testing::Return(cv::Mat(1, 5, CV_32F)));
            ON_CALL(*mock_, getDepthCameraMatrix()).WillByDefault(
                testing::Return(cv::Mat(3, 3, CV_32F)));
            ON_CALL(*mock_, getDepthDistortionMatrix()).WillByDefault(
                testing::Return(cv::Mat(1, 5, CV_32F)));

            ON_CALL(*mock_, getDepth2ColorExtrinsics()).WillByDefault(
                testing::Return(cv::Mat(4, 4, CV_32F)));
    }
 ...
 private:
    std::shared_ptr<crf::sensors::rgbdcameras::RGBDCameraMock> mock_;
